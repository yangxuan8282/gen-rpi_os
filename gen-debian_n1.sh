#!/bin/sh
#genetate debian [phicomm-n1] image: chmod +x gen-debian_n1.sh && sudo ./gen-debian_n1.sh
...
usage() {
	...
	Usage: gen-debian_n1.sh [options]
	...
	Valid options are:
		...
		-o OUTPUT_IMG           Output img file
					(default is BUILD_DATE-debian-n1--DEBIAN_BRANCH.img).
...
: ${OUTPUT_IMG:="${BUILD_DATE}-debian-n1-${DEBIAN_BRANCH}.img"}

#=======================  F u n c t i o n s  =======================#

gen_image() {
	fallocate -l $(( 1024 * 1024 *1024 )) "$OUTPUT_IMG"
cat > fdisk.cmd <<-EOF
	o
	n
	p
...
do_debootstrap() {
	...
	#local url="https://github.com/yangxuan8282/gen-rpi_os/releases/download/debian_rootfs/stretch-armhf_rootfs.tar.xz"
